  ...
  function generarResumen() {
    ...
    if (iso === "S") {
      ...
      if (typeof isoDescriptions !== "undefined") {
        const matchingIsoDesc = isoDescriptions.find(desc => desc.casoParticular === casoStr);
        if (matchingIsoDesc) {
      ...
      if (typeof descriptions !== "undefined") {
        ...
        const matchingDesc = descriptions.find(desc => {
          // Normalizamos el string almacenado quitándole todos los espacios en blanco
          const storedCase = desc.casoParticular.replace(/\s/g, "");
          return storedCase === casoStr;
        });
        if (matchingDesc) {
