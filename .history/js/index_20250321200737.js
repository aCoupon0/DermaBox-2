...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function encontrarProductos() {
    ...
    categorias.forEach(({ array, key }) => {
      const producto = array.find(item => {
        return item.configuraciones.some(config => {
          return JSON.stringify(config).replace(/\s/g, "") === casoStr;
        });
      });
    ...
    // Buscar en descriptions o isoDescriptions según el valor de iso
    let fuenteDescriptions = iso === "N" ? descriptions : isoDescriptions;
    const descripcionObj = fuenteDescriptions.find(desc => {
      return JSON.stringify(desc.casoParticular).replace(/\s/g, "") === casoStr;
    });
    ...
    // Validar si todos los valores en localStorage existen y no están vacíos
