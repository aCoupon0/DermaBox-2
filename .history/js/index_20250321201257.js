...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let quizOptions = [
    ...
    //PREGUNTA 2
    `
      <div class="quizLabel">¿Cuáles son tus objetivos?</div>
  ...
  function encontrarProductos() {
    console.log("Ejecutando encontrarProductos...");

    // Convertir casoParticular a string para la comparación
    const casoStr = JSON.stringify(casoParticular).replace(/\s/g, "");
    console.log("Caso Particular String:", casoStr);

    // Limpiar valores del localStorage antes de la búsqueda
    ...
    let productosEncontrados = 0; // Contador de productos encontrados

    categorias.forEach(({ array, key }) => {
        const producto = array.find(item => 
            item.configuraciones.some(config => JSON.stringify(config).replace(/\s/g, "") === casoStr)
        );
    ...
    let fuenteDescriptions = iso === "N" ? descriptions : isoDescriptions;
    const descripcionObj = fuenteDescriptions.find(desc => 
        JSON.stringify(desc.casoParticular).replace(/\s/g, "") === casoStr
    );

    if (descripcionObj) {
        console.log("Descripción encontrada:", descripcionObj.descripcionesFinales);
        localStorage.setItem("descripcionGlobal", JSON.stringify(descripcionObj.descripcionesFinales));
    } else {
        console.log("Descripción global no encontrada.");
    }

    // Validar que TODOS los valores existen antes de redirigir
    const todoEncontrado = todasLasClaves.every(key => {
        const item = localStorage.getItem(key);
        return item && item !== "null" && item !== "undefined" && item !== "{}";
    });

    if (todoEncontrado && productosEncontrados > 0) {
        console.log("Todos los productos y descripción global encontrados, redirigiendo a packs-picker.html...");
        window.location.href = "packs-picker.html"; // Redirigir a la página de selección de packs
    } else {
        console.log("Faltan productos o descripción, no se redirige.");
    }
}
