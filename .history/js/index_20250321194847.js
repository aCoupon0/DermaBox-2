...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function encontrarProductos() {
    const casoParticularStr = JSON.stringify(casoParticular);
    console.log("Buscando productos para casoParticular:", casoParticularStr);
    ...
    categorias.forEach(({ array, key }) => {
      const producto = array.find(item =>
        item.configuraciones.some(config => JSON.stringify(config) === casoParticularStr)
      );

      if (producto) {
        console.log(`Encontrado en ${key}:`, producto);
        const { configuraciones, ...productoSinConfig } = producto; // Copia sin 'configuraciones'
